...
omejitve so bile kršene ter kolikokrat se kršitev pojavi. S \emph{k\textsubscript{n}} označimo število
kršitev omejitve \emph{n}, z \emph{u\textsubscript{n}} pa njeno utež. Z naslednjo formulo izračunamo
kakovost urnika:
\[
   k_1 \cdot u_1 + k_2 \cdot u_2 + \dots + k\textsubscript{n-1} \cdot u\textsubscript{n-1} + k_n \cdot u_n
\]
...
\begin{enumerate}
   ...
   \item Z zanko se sprehodimo po vektorju vseh predmetov. Predmet na \emph{i}-tem koraku označimo s $predmet_i$.

   \item Naslednja vgnezdena zanka nas popelje po vseh predavalnicah. Predavalnico na
   \emph{j}-tem koraku označimo s $predavanica-j$.

   \item Za vse proste termine $predavalnice_j$ izračunamo ceno vstavljanja
   $predmeta_i$, ki ga sestavlja $predmeta_i$ , profesor, ki ta predmet predava,
   ...
   vstavljanja, da bomo $predmeta_i$ na koncu vstavili v termin z najboljšo
   ceno vstavljanja.
...
\end{enumerate} % konec okolja enumerateS
%
% Spodnja vrstica ne začne novega odstavka, zato ne bi smela biti zamaknjena.
% To popravite z ukazom `\noindent` ali pa tako, da odstranite prazno vrstico.
% (Nekoliko lepše izgleda, če v prazno vrstico vstavite % za komentar.)
...
Naj bo \emph{n}=100 ter $\emph s_1$ sprememba urnika, ki nas je pripeljala v lokalni maksimum. To
spremembo (npr. zamenjava $srečanja_i$ in $srečanja_j$) sedaj shranimo v
seznam tabujev. V naslednjem koraku izvedemo spremembo $\emph s_2$, ki se prav tako shrani v
...
se sprememba $\emph s_1$, ki vodi do lokalnega maksimuma, šele tedaj izbriše iz seznama
